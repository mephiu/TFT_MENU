...
void set_datetime_screen(uint8_t activeSymbol) {
	hagl_clear_screen();
	for (int i = 0; i < 5; ++i) {
		hagl_draw_rounded_rectangle(i, i, LCD_WIDTH - i, LCD_HEIGHT - i, 5 - i,
				rgb565(255, 0, 0));
	...
	hagl_put_text("Wybierz aktualna date i godzine ", 15, 30, rgb565(0, 102, 204),
			font5x7);
//	hagl_put_text("  usredniac pomiar", 15, 40, rgb565(0, 102, 204), font5x7);
	switch (activeSymbol) {
		case 0:
			snprintf((char*)os_buffer, 4, "%u", 5);
			hagl_put_text((char*) os_buffer, 60, 70, rgb565(0, 102, 204), font6x9);
